...
class xi_plus:
    ...
    """
    def __init__(self,n,zlim=(0,10),Nz=20,
                 Nell=5E4,cosmo=None,**kwargs):
        if cosmo is None:
            cosmo = Cosmology(**kwargs)
        self.ell = numpy.logspace(-1,8,Nell)
        P_ell = Pspec(self.ell,[n],zlim,Nz,cosmo)[0][0]
    ...
    def __call__(self,theta):
        ...
        dx = numpy.log(self.ell[1]) - numpy.log(self.ell[0])
        return (0.5/numpy.pi) * integrate.simps(integrand*self.ell, dx=dx )
...
class xi_circ:
    ...
    """
    def __init__(self,n,zlim=(0,10),theta_R=3.0,
        ...
        if cosmo is None:
            cosmo = Cosmology(**kwargs)
        self.ell = numpy.logspace(-1,8,Nell)
        self.theta_R = theta_R * numpy.pi/180./60.
    ...
    def __call__(self,theta):
        ...
        dx = numpy.log(self.ell[1]) - numpy.log(self.ell[0])
        return (2./numpy.pi/self.theta_R**2) * \
...
def Sample_xi(xi,Dmin,Dmax,N):
    ...
    """
    breaks = 10**numpy.arange(-3.,4.)
    breaks = breaks[numpy.where( (breaks>Dmin) & (breaks<Dmax) )]
    breaks = numpy.concatenate( ([Dmin],breaks,[Dmax]) )
    N_segments = len(breaks)-1
    N_per_segment = int( N/N_segments )
    assert N_per_segment>1

    theta = numpy.concatenate( [numpy.linspace(breaks[i],
                                               breaks[i+1],
                                               N_per_segment+1)[:-1] \
                                    for i in range(N_segments)] )

    corr = numpy.asarray( [xi(t) for t in theta] )
...
if __name__ == '__main__':
    ...
    theta = numpy.logspace(0,2.5,100)
    xi_p = xi_plus(n,zlim,Nz=20,cosmo = cosmo)
    xi_m = xi_minus(n,zlim,Nz=20,cosmo = cosmo)
    xi_R = xi_circ(n,zlim,theta_R=3.0,Nz=20,cosmo = cosmo)
    
    xi_p_t = numpy.asarray( [xi_p(t) for t in theta] )
    xi_m_t = numpy.asarray( [xi_m(t) for t in theta] )
    xi_R_t = numpy.asarray( [xi_R(t) for t in theta] )
