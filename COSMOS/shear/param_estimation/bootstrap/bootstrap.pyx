...
def bootstrap_resample(np.ndarray[CTYPE_t, ndim=1] gamma not None,
                       np.ndarray[DTYPE_t, ndim=1] RA not None,
                       np.ndarray[DTYPE_t, ndim=1] DEC not None,
                       DTYPE_t RAmin, DTYPE_t dRA, ITYPE_t nbinsRA,
                       DTYPE_t DECmin, DTYPE_t dDEC, ITYPE_t nbinsDEC,
                       ITYPE_t N_bootstraps):
    """
    ...
    """    
    cdef ITYPE_t i, j, k, i1, i2
    cdef CTYPE_t g
    cdef ITYPE_t Ngal = len(gamma)
    assert len(RA) == Ngal
    assert len(DEC) == Ngal

    #find RA indices and DEC indices
    cdef np.ndarray iRA = np.floor((RA - RAmin) / dRA).astype(int)
    cdef np.ndarray iDEC = np.floor((DEC - DECmin) / dDEC).astype(int)

    #compute the mean and number of galaxies in each shear bin
    cdef np.ndarray gamma_mean = np.zeros((nbinsRA, nbinsDEC), dtype=complex)
    cdef np.ndarray Ngal_bins = np.zeros((nbinsRA, nbinsDEC), dtype=int)
    ...
    cdef np.ndarray hash_list = iRA + power * iDEC
    cdef np.ndarray i_sort = np.argsort(hash_list)
    cdef np.ndarray gamma_list = gamma[i_sort]
    ...
    cdef np.ndarray indices = np.concatenate([[0], Ngal_bins.reshape(nbinsRA*nbinsDEC).cumsum()])
    
    #now compute the mean and standard deviation in a bootstrap resampling
    cdef np.ndarray gamma_sum = np.zeros(nbinsRA*nbinsDEC, dtype=complex)
    cdef np.ndarray gamma2_sum = np.zeros(nbinsRA*nbinsDEC, dtype=float)

    cdef np.ndarray ind
    
    for i from 0 <= i < N_bootstraps:
        for j from 0 <= j < nbinsRA*nbinsDEC:
            i1 = indices[j]
            i2 = indices[j+1]
            if i1==i2: continue
    ...
    cdef np.ndarray d2gamma = gamma2_sum - abs(gamma_sum**2)
