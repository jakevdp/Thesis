...
if __name__ == '__main__':
    ...
    pylab.hist(sigma, 50, normed=True, histtype='stepfilled', alpha=0.5)
    pylab.plot(xrange, yrange, '-k')
    
    pylab.xlabel(r'${\rm \sigma_{int}}$')
    pylab.ylabel(r'${\rm dN/d\sigma_{int}}$')

    pylab.text(0.05, 0.95, r'$\mathrm{mean\ \sigma_{int}\ =\ %.2f \pm %.2f}$'
               % (mu, s),
               ha='left', va='top',
               transform = pylab.gca().transAxes,
               fontsize=16)

    pylab.savefig('fig/fig01_sigma_calc.pdf')
    pylab.show()
