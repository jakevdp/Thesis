...
collapsed structure as a function of $z$ can
be used to place constraints on the components which make up the Universe.

For the special case of a matter-dominated universe, pressure is negligible
and we can use $\rho_b = \Omega_M\rho_c$
in eqn.~\ref{eq:delta_evolution} to find
\begin{equation}
...
In general, eqn.~\ref{eq:delta_evolution} can be solved by expressing the
density contrast in terms of the linear growth factor
$\delta(\myvec{r}, t) = \delta_0(\myvec{r})D(t)$ where $D$ is given by
\begin{equation}
  D(z) = \frac{H(z)\int_z^\infty dz^\prime(1 + z^\prime)H^{-3}(z^\prime)}
              {H_0\int_0^\infty dz^\prime(1 + z^\prime)H^{-3}(z^\prime)}
\end{equation}
